...
# -*- coding: utf-8 -*-
# ============================================================================
#  判据资产 - 片BU-R3：**局部避障"换向迟滞"的离线复算**（只读产物，⛔ 不进 Play）
#
#  为什么需要它（本片任务书 §2/§3）：
#    片BU-R2 把残余根因定位到 `BotNavigator.Avoid`：它**每帧**按 AvoidAngles 的**固定顺序**重扫候选，
#    于是"落地方向"在两个相距 180° 的候选之间以 ~10 次/秒 的速率翻（产品自己的 _diagFlips 计数：
#    Darrell 0.5s 内 +5，Scuzzy 349 次/回合）。任务书要求迟滞参数**有离线复算依据**，不许"觉得合适"。
#
...
#
#  ⚠️ 开环声明：② 用的是**已记录**的位置序列（它们本身就是"现状策略"的产物），不是闭环仿真 ——
...
def step(state, px, pz, desired, now, policy, bad_dir_s, switch_confirm_s, hold_s, order_mode):
    ...
    # （⛔ 本片不引入独立的"换向确认窗"：一维扫描显示 sw 0~0.4s 换向率不变 ⇒ 少一个状态量）
