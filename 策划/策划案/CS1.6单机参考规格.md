...
> `策划/实体清单.tsv` / `策划/状态矩阵.tsv`（覆盖矩阵族数据）**已于 2026-09-24 删除**。⛔ 覆盖矩阵 / 档位对账不再是交付闸门。
>
...
3. **地图口径**：几何**从原版 `de_dust2.bsp` 解析**（lump 几何 + 内嵌 miptex + 实体），
  ...
  > 数字口径（2026-09-21 切片F 回写）：阻挡盒**只**由离线量法产出（按"人体高度带"逐格判定后贪心合并），
  ...
  > （`∀ f ≤ f0+StepUpHeight`，`f0` = 该格最低地面候选），
  > 逐格重判后阻挡格变多、贪心合并出的矩形也随之增加 ⇒ 656 → 811。判据 = 离线断言（A1 / D9 行）。
  > 材质组 = `geo.bin` 的组数 = 场景 `Visual/*` 的 MeshFilter 节点数 = **33**。判据见离线断言（A1/A2）。
