...
> **规模档位：L 档**（1:1 复刻 / 多系统）。**判一次、只许上调**，⛔ 不许因赶时间下调。
> ⚠️ **口径已更新（2026-09-24）**：覆盖矩阵 / 档位对账 / 逐格填证据这套**已从 skill 删除**（验收 = 产出为主）。
> 落地方式 = `策划/验收表.md` 的**系统清单**（一行一个系统，`没做` / `做了`）+ 交付前**关键路径跑一次 + 看一眼截图**。
> `策划/实体清单.tsv` / `策划/状态矩阵.tsv`（覆盖矩阵族数据）**已于 2026-09-24 删除**；`（一次性判据脚本，2026-09-24 清理）` 属 `tools/probes/`，其存废在该目录一并处理。⛔ 覆盖矩阵 / 档位对账不再是交付闸门。
>
...
3. **地图口径**：几何**从原版 `de_dust2.bsp` 解析**（lump 几何 + 内嵌 miptex + 实体），
  ...
  > 数字口径（2026-09-21 切片F 回写）：阻挡盒**只**由 `（一次性判据脚本，2026-09-24 清理）` 产出（按"人体高度带"逐格判定后贪心合并），
  ...
  > （`∀ f ≤ f0+StepUpHeight`，`f0` = 该格最低地面候选；`（一次性判据脚本，2026-09-24 清理）` 文档 §④），
  > 逐格重判后阻挡格变多、贪心合并出的矩形也随之增加 ⇒ 656 → 811。判据 = `（一次性判据脚本，2026-09-24 清理）`（A1）+ `（一次性判据脚本，2026-09-24 清理）` 的 D9 行。
  > 材质组 = `geo.bin` 的组数 = 场景 `Visual/*` 的 MeshFilter 节点数 = **33**。判据见 `（一次性判据脚本，2026-09-24 清理）`（A1/A2）。
